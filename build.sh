...
## These needs to be exported
#export ARCH=arm64
#export SUBARCH=ARM64
#export CROSS_COMPILE=aarch64-linux-gnu-
#export CC=$TOOLLCHAIN_DIR/$CLANG_VERSION/bin/clang
#export LD=ld.lld
#export LLVM=1 LLVM_IAS=1
# Build tools
#export PATH=$TOOLLCHAIN_DIR/build-tools/path/linux-x86:$PATH
#export PATH=$TOOLLCHAIN_DIR/gas/linux-x86:$PATH
...
setup_env(){
    ...
    echo -e "${green}Environment set up!${nocol}"
}
...
SUSFS_Patch() {
    ...
    if [[ "$PATCH_SUSFS" == "1" ]]; then
        ...
        if patch -p1 --fuzz=3 < 50_add_susfs_in_gki-android12-5.10.patch; then
